...
LastEditTime: 2025-04-17 18:25:02
Description: 
'''
import platform
...
class MultiModel(nn.Module):
    ...
    -----------
    models: 
        - CNN Encoder: MATTPE with padding, cnn_cfg and each layer RoPE settings
        - SMILES Encoder: PredTokenAttn(n_token=1)
        - Cross Attntion: CrossAttn
        - wet_predictor: PredTokenAttn(n_token=out_dim1)+MLP
        - plip_redictor: SimpleMLP
    """
    def __init__(self, args: GlobalSettings, cnn_cfg: list[LayerCfg], padding: int, hidden_dim: int, out_dim1: int, out_dim2: int,
                 n_layer: int = 1, n_head: int = 8, dropout: float = 0.4,
                 RoPE: list[bool] = None):
        super().__init__()
        # CNN Encoder
        self.pose_cnn = CNN2D(args, cnn_cfg, padding, hidden_dim)
