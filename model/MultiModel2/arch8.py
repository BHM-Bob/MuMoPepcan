...
LastEditTime: 2025-06-26 22:15:46
Description: 
'''
import platform
...
from model.predictor import MHMLP, SimpleMLP, MCMLP
...
class MultiModel(nn.Module):
    def __init__(self, args: GlobalSettings, cnn_cfg: list[LayerCfg], hidden_dim: int, out_dim1: int, out_dim2: int,
        ...
        -----------
        models: 
            - CNN Encoder: COneD, cnn_cfg and each layer RoPE settings
            - SMILES Encoder: PredTokenAttn(n_token=1)
            - Cross Attntion: CrossAttn
            - wet_predictor: PredTokenAttn(n_token=4)+MCMLP(4)
        ...
        self.wet_proj = MCMLP(out_dim1, hidden_dim, dropout)
    ...
    def forward(self, pose: torch.Tensor, smiles: torch.Tensor, mask: Optional[torch.Tensor] = None):
        ...
        # multi-task predict
        wet_p = self.wet_proj(self.wet_predictor(aligned_smiles_feat, mask=None)) # [N, out_dim]
